...
def main():
	...
	if tune_flag:
		...
		RandomForestClassifier(min_samples_leaf = best_model.best_estimator_.min_samples_leaf, 
	...
	plot_confusion_matrix(y_train, pred_train, classes=np.unique(y_train), normalize=False,
                      title='RF training confusion matrix')
	plt.savefig('RF_tr_cm')

	plot_confusion_matrix(y_validate, pred_validate, classes=np.unique(y_train), normalize=False,
                      title='RF testing confusion matrix')
	plt.savefig('RF_ts_cm')
